...
int removeIdenticalStrains(int number_of_strains, int length_of_MSA, int** MSA, int* identical, char** names_of_strains, int maxname, Options opt, int ref_index, int start_of_ref, int end_of_polyA, int* strains_remaining, int* reference){
	int i,j, k;
	int mismatch=0;
	int index=0;
	/*printf("finding variant sites\n");
	int* variant_sites = (int*)malloc(length_of_MSA*sizeof(int));
	for(i=0; i<length_of_MSA; i++){
		variant_sites[i]=-1;
	}
	int number_of_variant_sites=0;
	for(i=0; i<length_of_MSA; i++){
		int base=MSA[0][i];
		int invariant=0;
	...
	//char** temp_name = (char**)malloc(number_of_strains*sizeof(char*));
	//for(i=0; i<number_of_strains; i++){
	//	temp_name[i]=(char*)malloc(10000*sizeof(char));
	//	for(j=0; j<10000; j++){
	//		temp_name[i][j]='\0';
	//	}
	//}
	...
	//}
	for(i=0; i<number_of_strains; i++){
		for(j=0; j<end_of_polyA+1; j++){
			if ( reference[j] == -1 ){ break; }
			if ( MSA[i][reference[j]] == 0 ){
				seq[i][j] = '-';
			}else if (MSA[i][reference[j]] == 1 ){
				seq[i][j] = 'A';
			}else if (MSA[i][reference[j]] == 2 ){
				seq[i][j] = 'G';
			}else if (MSA[i][reference[j]] == 3 ){
				seq[i][j] = 'C';
			}else if (MSA[i][reference[j]] == 4 ){
				seq[i][j] = 'T';
			}else if (MSA[i][reference[j]] == -1 ){
				seq[i][j] = 'N';
			}
		...
		//strcpy(temp_name[i],(char*)hashmap_get(&hash, seq[i]));
		struct blob *c;
		c=hashmap_get(&hash, seq[i]);
		if (c==NULL){
			struct blob *b;
			b = malloc(sizeof(*b));
			b->key = seq[i];
			b->data = names_of_strains[i];
			b->data_len = strlen(names_of_strains[i]);
			...
			c->data[k] = ':';
			strcat(c->data,names_of_strains[i]);
...
void findVariantSites(int number_of_strains, int length_of_MSA, int** MSA, int* invariant_sites, Options opt, int ref_index, int start_of_ref, int end_of_polyA, int* strains_remaining, int* reference){
	...
	for(i=0; i<end_of_polyA+1; i++){
		...
		if ( invariant_sites[reference[i]] == -1 ){
			number_of_sites++;
		}
	}
	FILE *invariant_file;
...
int main(int argc, char **argv){
	...
	for(i=0; i<number_of_strains; i++){
		...
		names_of_strains[i] = (char *)malloc((10000)*sizeof(char));
		for(j=0; j<10000; j++){
			names_of_strains[i][j] = '\0';
		}
	}
	...
	//if (opt.remove_identical==1){
		...
		number_of_identical_strains=removeIdenticalStrains(number_of_strains,length_of_MSA,MSA,identical,names_of_strains,max_name_length,opt,ref_index,start_of_ref,end_of_ref,strains_remaining,reference);
		clock_gettime(CLOCK_MONOTONIC, &tend);
		printf("Took %.5fsec\n",((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec));
	}
	int* variant_sites = (int *)malloc(length_of_MSA*sizeof(int));
	if (opt.print_variant==1){
		...
		findVariantSites(number_of_strains,length_of_MSA,MSA,variant_sites,opt,ref_index,start_of_ref,end_of_ref,strains_remaining,reference);
		//free(variant_sites);
	}
	//FILE* new_ref_file;
