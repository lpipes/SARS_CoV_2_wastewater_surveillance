...
int process_problematic_sites(int* problematic_sites){
	FILE* file;
	if (( file = fopen("problematic_sites_sarsCov2.vcf","r")) == (FILE *) NULL ) fprintf(stderr, "Cannot find problematic_sites_sarsCov2.vcf file, please make sure it is in this directory.\n");
	char buffer[1000];
	char name[30];
	int position;
	char ch1[1];
...
int main(int argc, char **argv){
	...
	align_references(number_of_problematic_sites,problematic_sites,opt.MSA_reference,opt.bowtie_reference_db);
	if (( MSA_file = gzopen(opt.fasta,"r")) == Z_NULL ) fprintf(stderr, "File could not be opened.\n");
