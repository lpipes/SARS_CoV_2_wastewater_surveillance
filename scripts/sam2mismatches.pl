...
if (scalar(@ARGV) < 2){
	print "usage:\n";
	print "sam2mismatches.pl MSA.fasta alignment.sam\n";
		...
		if ($chrom ne "*"){
			...
			foreach my $key ( sort keys %reference_genomes ){
				...
				for(my $i=0; $i<scalar(@positions)-1; $i++){
					$j=$i+$qStart;
					if ( uc(@{$reference_genomes{$key}}[$positions[$i]]) ne uc($sequence_chars[$j]) and @{$reference_genomes{$key}}[$positions[$i]] ne "-" and uc(@{$reference_genomes{$key}}[$positions[$i]]) ne "N"){
