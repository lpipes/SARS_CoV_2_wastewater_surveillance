...
#include "global.h"
void align_references(int number_of_problematic_sites, int* problematic_sites, char MSA_reference[], char Wuhan_reference[]){
	FILE* Alignment_ref;
	char* line = NULL;
	size_t len = 0;
	ssize_t read;
	...
	if (Alignment_ref == NULL){
		printf("Error cannot find MSA reference index file!");
		exit(1);
	}
	...
	FILE* Wuhan_file;
	Wuhan_file = fopen(Wuhan_reference,"r");
	if (Wuhan_file == NULL){
		printf("Error cannot find Wuhan reference file supplied with -d!");
		exit(1);
	}
